...
namespace KWiJisho.Commands.Prefix
{
    ...
    internal partial class PrefixCommandManager
    {
        ...
        internal class PrefixInfo : BaseCommandModule
        {
            ...
            [Command(nameof(help))]
            internal async Task GetHelpAsync(CommandContext commandContext) => await Commands.Info.ExecuteHelpAsync(commandContext.Channel, commandContext.Client);
            ...
            /// <returns>A <see cref="Task"/> from the current asynchronous task.</returns>
            [Command(nameof(info))]
            internal async Task GetInfoAsync(CommandContext commandContext) => await Commands.Info.ExecuteInfoAsync(commandContext.Channel, commandContext.Client);
        }
    }
}
